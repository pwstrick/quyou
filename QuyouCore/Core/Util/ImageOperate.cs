...
namespace QuyouCore.Core.Util
{
    public class ImageOperate
    {
        ...
        public static string LetterWatermark(string path, int size, string letter, Color color, string location)
        {
            ...
            {
                DateTime time = DateTime.Now;
                string filename = string.Format("{0}{1}{2}{3}{4}{5}{6}", time.Year.ToString(), time.Month.ToString(), time.Day.ToString(), time.Hour.ToString(), time.Minute.ToString(), time.Second.ToString(), time.Millisecond.ToString());
                Image img = Bitmap.FromFile(path);
